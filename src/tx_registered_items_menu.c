...
static void TxRegItemsMenu_PrintItemIcon(enum Item itemId);
...
static void TxRegItemsMenu_CopyItemName(u8 *string, enum Item itemId);
...
#define TAG_INDICATOR_ARROWS      TAG_ITEM_ICON
...
static const struct ListMenuTemplate gTxRegItemsMenu_List =
{
    .items = NULL,
...
static void TxRegItemsMenu_ClearAndInitData(u8 taskId)
{
    ...
    TxRegItemsMenuItemPageInfo.pageItems = 3;
    TxRegItemsMenu_AllocateStruct();
...
static void TxRegItemsMenu_InitDataAndCreateListMenu(u8 taskId)
{
    ...
    TxRegItemsMenu_CalculateUsedSlots();
    TxRegItemsMenu_CalcCursorPos();
...
static void TxRegItemsMenu_MoveCursor(s32 id, bool8 b, struct ListMenu *thisMenu)
{
    ...
    if (gTxRegItemsMenu->toSwapPos == 0xFF)
    {
        ...
        if (id != -2)
            ...
            TxRegItemsMenu_PrintItemIcon(ITEM_LIST_END);
    }
}
...
static void TxRegItemsMenu_DoItemSwap(u8 taskId, bool8 a)
{
    ...
    enum Item lastSelectedItemId = gSaveBlock1Ptr->registeredItems[lastSelected].itemId;
...
static void TxRegItemsMenu_StartScrollIndicator(void)
{
    if (TxRegItemsMenuItemPageInfo.scrollIndicatorTaskId == TASK_NONE)
        TxRegItemsMenuItemPageInfo.scrollIndicatorTaskId = AddScrollIndicatorArrowPairParameterized(SCROLL_ARROW_UP, 28, 110, 148, TxRegItemsMenuItemPageInfo.count - TxRegItemsMenuItemPageInfo.pageItems, TILE_TAG_INDICATOR_ARROWS, TAG_INDICATOR_ARROWS, &(TxRegItemsMenuItemPageInfo.itemsAbove));
}
...
bool8 TxRegItemsMenu_AddRegisteredItem(enum Item itemId)
{
    ...
    if (freeSlot == -1) // no slot left, return (triggers error message)
    {
        Free(newItems);
        return FALSE;
    }
