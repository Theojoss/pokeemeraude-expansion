...
SINGLE_BATTLE_TEST("Supersweet Syrup lowers evasion once per battle by one stage")
{
    GIVEN {
        ...
        MESSAGE("Le nectar de Pomdramour ennemi dégage un parfum sucré!");
        ANIMATION(ANIM_TYPE_GENERAL, B_ANIM_STATS_CHANGE, player);
        MESSAGE("2 retire Pomdramour!");
        MESSAGE("2 retire Qulbutoké!");
        NONE_OF {
            ABILITY_POPUP(opponent, ABILITY_SUPERSWEET_SYRUP);
            ANIMATION(ANIM_TYPE_GENERAL, B_ANIM_STATS_CHANGE, player);
            MESSAGE("Le nectar de Foes Pomdramour dégage un parfum sucré!");
        }
    } THEN {
...
SINGLE_BATTLE_TEST("Supersweet Syrup can not further lower opponents evasion if it is at minimum stages")
{
    GIVEN {
        ...
        NONE_OF {
            ANIMATION(ANIM_TYPE_GENERAL, B_ANIM_STATS_CHANGE, player);
            MESSAGE("Ah, esquive du Mystherbe baisse!");
        }
        MESSAGE("esquive de Mystherbe ne peut plus baisser!");
    } THEN {
